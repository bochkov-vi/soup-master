...
public class SimulatorsView extends InteractiveMaterialsView {
	...
	private void initTree() {
		...
		List<Simulator> simulatorList = simulatorRepository.findAllByTemplateNotNullAndIsDeletedIsFalse().stream()
	...
	private void openSelectTemplateDialog() {
		...
		List<Simulator> simulatorList = simulatorRepository.findAllByIsDeletedIsFalse();
	...
	public void updateTreeData() {
		simulatorTreeData.clear();
		roots = simulatorRepository.findAllByTemplateNotNullAndIsDeletedIsFalse().stream().sorted(Comparator.comparingLong(Simulator::getId))
				.collect(Collectors.toList());
