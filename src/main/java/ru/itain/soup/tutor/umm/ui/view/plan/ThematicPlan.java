...
public class ThematicPlan extends CommonView {
    ...
    private void openDisciplineEditDialog() {
        List<Discipline> disciplines = disciplineRepository.findAll(tutorRepository.getCurrentDepartment())
                .stream()
                .sorted(Comparator.comparingLong(Discipline::getId))
                .collect(Collectors.toList());
        new SoupElementWithDepartmentEditDialog<Discipline>(disciplines, departmentRepository.findAll(), tutorRepository.getCurrentDepartment(), "Редактирование дисциплины") {
            @Override
            protected void updateElementList() {
                ummTreeCreator.refreshAll();
            }
