...
import static ru.itain.soup.common.ui.component.MaterialDiv.*;
...
public class UmmInfoLayout extends VerticalLayout implements UmmChangedNotifier {
    ...
    private EditableMaterialDiv getEditableMaterialDiv(
        ...
        List<Test> testList = StreamSupport.stream(testRepository.findAll(tutorRepository.getCurrentDepartment()).spliterator(), false).collect(Collectors.toList());
        List<Article> articlesList = StreamSupport.stream(articleRepository.findAll(tutorRepository.getCurrentDepartment()).spliterator(), false).collect(Collectors.toList());
        List<Presentation> presentationsList = new ArrayList<>(presentationRepository.findAll(tutorRepository.getCurrentDepartment()));
        List<Simulator> simulatorsList = new ArrayList<>(simulatorRepository.findAllByTemplateNotNullAndIsDeletedIsFalse(tutorRepository.getCurrentDepartment()));
