...
public class ArticleMainView extends CommonView {
	...
	private void updateArticlesMap() {
		List<Article> collect = StreamSupport.stream(articleRepository.findAll().spliterator(), false).collect(Collectors.toList());
	...
	private void openThemeEditDialog() {
		new SoupElementEditDialog<Article>(articleRoots, "РЕДАКТИРОВАНИЕ ТЕМ") {
			@Override
